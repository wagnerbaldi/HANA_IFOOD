...
try {
	...
	*/
	...
	sqlstmt = "INSERT INTO \"IFOOD\".\"HANA_IFOOD.db.data::ZoomMeetingDetail\" VALUES( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)" ;
    ...
    for ( var i in jobj.meetings.root)  {
		...
		st.setString(13,jobj.meetings.root[i].dept);
		st.setString(14,jobj.meetings.root[i].join_url);
		st.setString(15,jobj.meetings.root[i].created_at);
		st.setString(16,jobj.meetings.root[i].timezone);
